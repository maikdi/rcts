...
accounts_txt = open('infolomba.txt', "r")
...
def log_to_db(table, username, url, caption):
    today = str(date.today().strftime('%Y-%m-%d'))
    now = str(time.strftime("%H:%M:%S"))
    cursor.execute(
        f"INSERT INTO {table}(log_date, log_time, username, url, caption)"
        f"VALUES('{today}', '{now}', '{username}', '{url}', '{caption}')")
    commit()
...
def output(account_username, data):
    ...
    log_to_db("testing", account_username, post_url, post_caption)
...
if __name__ == "__main__":
    ...
    for i in range(len(accounts_list)):
        ...
        output(args.username, get_posts_by_user(args.username, args.number, args.mode == "posts_full"))
        acquired_posts += 1
    print(f"Done, acquired {acquired_posts} posts.")
